...
public class SQLHandler {
    ...
    public boolean registerlogin(String login, String password) {
        try {
            Connection connection = DriverManager.getConnection("jdbc:mysql://localhost:3306/login_information",
                    "root", "1234");
            if (!checkUserExist(connection, login)) {
                createUser(connection, login, password);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void createUser(Connection connection, String login, String password) {
        try {
            Statement statement = connection.createStatement();
            statement.executeUpdate("insert into login_information.user_information (username, passwd) values ('" + login + "', '" + password + "')");
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    public boolean checkUserExist(Connection connection, String login) {
        try {
            Statement statement = connection.createStatement();
